    ...
    def get(self, request, *args, **kwargs):
        if request.GET:
            ...
            if not date_rate or not currency:
                params['message'] = 'Дата и валюта должны быть заданы.'
...
class RateView(generic.View):
    ...
    def get(self, request, date_rate, currency):
        ...
        try:
            ...
            text = f'Дата "{date_rate}" должна быть в формате ГГГГ-ММ-ДД.'
            data = {'message': text}
            status = HTTPStatus.BAD_REQUEST
        else:
            rates = Rate.objects.filter(date=date_rate, currency__code=currency)
            if len(rates) == 0:
                text = f'Данные по валюте "{currency}" на дату {date_rate} не загружены.'
                data = {'message': text}
    ...
    def post(self, request):
        ...
        if status == HTTPStatus.CREATED:
            text = f'Данные по валюте на дату {date_import} успешно загружены.'
        elif status == HTTPStatus.CONFLICT:
            text = f'Данные по валюте на дату {date_import} уже существуют в системе.'
        else:
            text = f'Данные по валюте на дату {date_import} отсутствуют в системе НБРБ.'
